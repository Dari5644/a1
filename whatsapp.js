...
import { BOT_SYSTEM_PROMPT, supportPhones } from "./config.js";
import {
  initDb,
  upsertContact,
  getContactByWaId,
...
async function askAI(userText) {
  try {
    ...
    return (
      reply ||
      "حصل خطأ بسيط في الرد الآلي، حاول تكتب سؤالك مرة ثانية 🙏"
    );
  } catch (err) {
...
async function handleIncomingMessage(fromJid, text, fromMe = false) {
  ...
  console.log("📩 رسالة من:", phone, "fromMe?", fromMe, "النص:", msg);
  ...
  const contact = await upsertContact(phone, phone);
  ...
  const needSupport =
    ...
    msg.includes("مو واضح") ||
    msg.includes("ما فهمت") ||
    msg.includes("وش تقصد") ||
    msg.includes("وضح أكثر") ||
  ...
  const freshContact = await getContactByWaId(phone);
...
export async function startWhatsApp() {
  try {
    ...
    console.error("❌ خطأ في startWhatsApp:", err);
  }
}
