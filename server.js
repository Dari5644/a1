...
// مسار بسيط للتأكد أن السيرفر شغال
app.get("/ping", (req, res) => {
  res.send("OK - Smart Bot server is running");
});

// ===================== Webhook GET (تحقق ميتا) =====================
...
// ===================== Webhook POST (رسائل واتساب من Meta) =====================
app.post("/webhook", async (req, res) => {
  console.log("🔥🔥 POST /webhook – استقبلنا طلب من Meta 🔥🔥");
  console.log("BODY:", JSON.stringify(req.body, null, 2));

  const body = req.body;

  if (!body || !body.entry) {
    console.warn("⚠️ Webhook بدون entry – سيتم الرد 200");
    return res.sendStatus(200);
  }

  try {
    if (body.object !== "whatsapp_business_account") {
      console.warn("⚠️ body.object ليس whatsapp_business_account:", body.object);
      return res.sendStatus(200);
    }

    const entry = body.entry[0];
    if (!entry || !entry.changes) {
      console.warn("⚠️ entry بدون changes");
      return res.sendStatus(200);
    }
    ...
    for (const change of entry.changes) {
      ...
      if (!messages.length) {
        console.log("ℹ️ لا توجد messages في هذا الحدث (ربما status فقط)");
        continue;
      }

      // هنا نفترض message واحد – كافي لغالب الحالات
      const msg = messages[0];
      const contactMeta = contactsMeta[0];

      const fromWaId = msg.from; // مثال: 9665xxxxxxxx
      const name =
        contactMeta &&
        contactMeta.profile &&
      ...
      console.log("📩 رسالة من:", fromWaId, "الاسم:", name, "النص:", text);
      ...
      if (
        ...
        console.log("✅ تم إرسال رسالة تشغيل البوت للعميل:", fromWaId);
        continue;
      }

      // 2) طلب خدمة العملاء صريح
      const wantsAgent =
      ...
      if (wantsAgent) {
        ...
        await sendWhatsAppMessage(fromWaId, reply);
        console.log("✅ تم تحويل العميل إلى خدمة العملاء:", fromWaId);
        continue;
      }

      // 3) هل البوت متوقف لهذا الرقم؟
      ...
      if (freshContact && freshContact.bot_paused) {
        ...
        await sendWhatsAppMessage(fromWaId, reply);
        console.log("ℹ️ العميل في وضع خدمة العملاء، تم تذكيره بكيفية تشغيل البوت:", fromWaId);
        continue;
      }
      ...
      if (seemsConfused) {
        ...
        await sendWhatsAppMessage(fromWaId, reply);
        console.log("ℹ️ العميل يبدو غير مستوعب، تم سؤاله عن التحويل لخدمة العملاء:", fromWaId);
        continue;
      }

      // 5) رد افتراضي (تعريفي + ذكاء اصطناعي)
      const intro =
      ...
      const aiReply = await askAI(clean || "رسالة فارغة من العميل");
      ...
      console.log("✅ تم إرسال رد تلقائي مع تعريف + AI للعميل:", fromWaId);
    }
...
app.post("/order-webhook", async (req, res) => {
  try {
    ...
    console.log("📦 /order-webhook body:", req.body);
